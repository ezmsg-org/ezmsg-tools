    ...
    async def count(self) -> typing.AsyncGenerator:
        ...
        while count < self.SETTINGS.iterations:
            msg = AxisArray(
                data=np.array([[count]]),
                dims=["time", "ch"],
                axes={"time": AxisArray.Axis.TimeAxis(fs=self.SETTINGS.approx_srate, offset=time.time())}
            )
...
class Sine(ez.Unit):
    ...
    async def on_message(self, message: AxisArray) -> typing.AsyncGenerator:
        yield self.OUTPUT_SIGNAL, replace(message, data=np.sin(2 * np.pi * 1.0 * message.data))
...
def main(runtime: float = 300.0, approx_srate: float = 5.0, graph_addr: str = "127.0.0.1:25978"):
